...
func (suite *TimeHelperTestSuite) TestTimeHelper_CalculateDuration() {
	suiteTest := suite.T()
	suiteTest.Run("string ends with d parsed as days", func(t *testing.T) {
		got, err := CalculateDuration("2d")
	...
	suiteTest.Run("parse as hms", func(t *testing.T) {
		got, err := CalculateDuration("10h38m23s")
